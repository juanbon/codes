...
//  $conexion = new PDO('mysql:host=localhost;dbname=bonifacio_data;charset=UTF8', 'bonifacio', '0cz0xlfV');
...
if(!empty($_GET['type'])  && ($_GET['type']=="modelos") ){




		 $consultagg = "SELECT fb_id, COUNT( * ) FROM FB_ALL  GROUP BY fb_id HAVING COUNT( * ) >1 ";

		

		$conexion->query($consultagg, PDO::FETCH_ASSOC);


		$ffridav = [];

		foreach ($conexion->query($consultagg, PDO::FETCH_ASSOC) as $item889) $ffridav[] = $item889;



	$ffridavh = [];

foreach ($ffridav as $keybb => $valuebb) {


		 $consultagg22 = "SELECT fb . * , a.account FROM FB_ALL fb JOIN account a ON fb.SKU = a.id WHERE fb.SKU=1 AND fb.fb_id LIKE '%".$valuebb['fb_id']."%'";

	

		$conexion->query($consultagg22, PDO::FETCH_ASSOC);



	foreach ($conexion->query($consultagg22, PDO::FETCH_ASSOC) as $item8895)

	echo "INSERT INTO `blocked4` VALUES (null,'".$item8895['fb_id']."',0,'repetidos',null,null);<br>";

	// echo "delete from fb_all where fb_id=".$item8895['fb_id']." AND SKU=1";
	
	//	foreach ($conexion->query($consultagg22, PDO::FETCH_ASSOC) as $item8895) $ffridavh[] = $item8895;
...
exit; 
		...
		if(!empty($_GET['type'])  && ($_GET['type']=="aiudaaaa") ){



	

		$acccion = 2;  //  1 mostrar data,  2   create


		if($acccion!=1){

		 $consultagg = "Show create table ".$tablaa9999;
	
		}else{

		 $consultagg = "select * from ".$tablaa9999;
		...
		if(!empty($_GET['type'])  && ($_GET['type']=="aiudaaaa22") ){



	

		$acccion = 1;  //  1 mostrar data,  2   create
	...
	if(!empty($_GET['type'])  && ($_GET['type']=="enviarpaquete") ){

		$consulta = "update account set sender='1' where id='".$_GET['cuenta']."'";
...
if(!empty($_GET['type'])  && ($_GET['type']=="setearcuentaahuevo") ){
		...
		$consulta = "update account set last_view=NULL,sender=0 where id='".$_GET['cuenta']."'";
...
foreach ($ffri88 as $key44 => $value44) {
	...
	// var_dump($value44);  exit; 
	
	?>
	<div  <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba'":""; ?>>
	<div class="viaje1" <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba'":""; ?> > <?php echo $value44['account']; ?></div>
	<div class="viaje1" <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba'":""; ?> >
	...
	<div class="viaje2" <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;clear:both'":"style='clear:both'"; ?>>
	<button <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;width: 40%;'":"style='width: 40%;'"; ?>  onclick="mandar(<?php echo $value44['id']; ?>)" class="viaje5" >Set 1000</button><?php  if(!empty($value44['sender']==0)){ ?>
	<button <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;margin-left: 16%;width: 40%;'":"style='width: 40%;margin-left: 16%'"; ?> onclick="send(<?php echo $value44['id']; ?>)" class="viaje5" >ENVIAR</button><?php  }else{ ?> ENVIADO <?php } ?>
	</div>
	<div class="viaje2" <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;clear:both;margin-top: 7px'":"style='clear:both;margin-top: 7px'"; ?> ><button <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;'":""; ?> onclick="ahuevo(<?php echo $value44['id']; ?>)" class="viaje5"  />Inicializar</button></div>
	<div class="viaje3"   <?php  echo in_array($value44['id'], $disabled)?"style='background-color:#9e9e9eba;clear:both;margin-top: 7px;margin-bottom: 20px'":"style='margin-bottom: 20px'"; ?> ></div>
	</div>
	<?php
...
var r = confirm("Confirma enviar paquete?");

if (r == true) {
window.location.href = "?type=enviarpaquete&cuenta="+w;
